# Training Configuration Parameters and Metrics

Last updated: 07/02/2026.

For NPU-related features, refer to the [NPU Advanced Features Guide](https://github.com/verl-project/verl/blob/main/docs/ascend_tutorial/zh/feature_support/npu_advance_features.md).

verl manages all parameters through a hierarchical YAML configuration file. All related configuration files are located in the `verl/trainer/config` directory.
...
| `actor_rollout_ref.actor.optim.total_training_steps` | `-1` | Total number of training steps; -1 indicates automatic calculation |
| `actor_rollout_ref.actor.optim.weight_decay` | `0.01` | Weight decay, used to prevent model overfitting |
| `actor_rollout_ref.actor.optim.lr_warmup_steps` | `-1` | Number of learning rate warmup steps; -1 indicates automatic calculation based on the ratio |
...
| `actor_rollout_ref.actor.clip_ratio` | `0.2` | PPO clipping ratio that controls the policy update magnitude. The typical value range is [0.1, 0.3]. |
...
| `actor_rollout_ref.actor.entropy_coeff` | `0` | Entropy regularization coefficient that controls the policy exploration degree. |
...
| `actor_rollout_ref.actor.entropy_from_logits_with_chunking` | `false` | Whether to calculate entropy from logits in chunks. |
...
| Parameter | Default Value | Description |
|-----------|----------------|--------------|
| `actor_rollout_ref.actor.policy_loss.loss_mode` | `vanilla` | Policy loss mode. Available options include vanilla, clip_cov, kl_cov, dppo_tv, dppo_kl, gspo, sapo, geo_mean, cispo, gpg, bypass_mode, and reinforce_is. |
...
| `actor_rollout_ref.rollout.top_k` | `-1` | Top-K sampling parameter. `-1` means disabled |
...
| `actor_rollout_ref.rollout.enforce_eager` | `false` | Whether to force the PyTorch eager mode |
...
| `actor_rollout_ref.rollout.max_model_len` | `null` | Maximum model sequence length. `null` means automatic inference |
...
| `actor_rollout_ref.rollout.logprobs_mode` | `processed_logprobs` | Logprobs calculation mode |
...
| `actor_rollout_ref.rollout.log_prob_micro_batch_size` | `null` | Micro batch size for log prob calculation |
| `actor_rollout_ref.rollout.log_prob_micro_batch_size_per_gpu` | `null` | Log prob micro batch size per GPU |
| `actor_rollout_ref.rollout.log_prob_use_dynamic_bsz` | Referenced from `actor.use_dynamic_bsz` | Whether log prob uses a dynamic batch size |
...
| `actor_rollout_ref.rollout.over_sample_rate` | `0` | Over-sampling rate |
...
| `actor_rollout_ref.rollout.val_kwargs.do_sample` | `false` | Whether to use sampling during validation |

#### 1.1.6 Multi-Turn Configuration

| Parameter Name | Default Value | Description |
|--------|--------|------|
| `actor_rollout_ref.rollout.multi_turn.enable` | `false` | Whether to enable multi-turn conversations |
...
| Parameter | Default Value | Description |
|-----------|----------------|-------------|
| `actor_rollout_ref.rollout.checkpoint_engine.backend` | `naive` | The backend of the checkpoint engine |
| `actor_rollout_ref.rollout.checkpoint_engine.update_weights_bucket_megabytes` | `2048` | The bucket size for weight updates (MB) |

#### 1.1.9 Trace Configuration

| Parameter | Default Value | Description |
|-----------|----------------|-------------|
| `actor_rollout_ref.rollout.trace.project_name` | Referenced from `trainer.project_name` | The tracing project name |
| `actor_rollout_ref.rollout.trace.experiment_name` | Referenced from `trainer.experiment_name` | The tracing experiment name |
| `actor_rollout_ref.rollout.trace.backend` | `null` | The tracing backend |
...
| `actor_rollout_ref.rollout.prometheus.file` | `/tmp/ray/session_latest/metrics/prometheus/prometheus.yml` | Path to the Prometheus configuration file |
...
| Parameter | Default Value | Description |
|--------|--------|------|
| `actor_rollout_ref.ref.rollout_n` | Inherited from `rollout.n` | Number of rollouts |
...
| `actor_rollout_ref.ref.log_prob_use_dynamic_bsz` | Inherited from `actor.use_dynamic_bsz` | Whether log prob uses dynamic batch size |
...
| `critic.ppo_mini_batch_size` | Inherited from `actor.ppo_mini_batch_size` | PPO mini-batch size |
...
| `critic.use_dynamic_bsz` | Inherited from `actor.use_dynamic_bsz` | Whether to use a dynamic batch size |
| `critic.ppo_max_token_len_per_gpu` | `32768` | Maximum PPO token length per GPU |
| `critic.forward_max_token_len_per_gpu` | Inherited from `critic.ppo_max_token_len_per_gpu` | Maximum token length per GPU for forward computation |
| `critic.ppo_epochs` | Inherited from `actor.ppo_epochs` | Number of PPO update epochs |
| `critic.shuffle` | Inherited from `actor.shuffle` | Whether to shuffle |
| `critic.data_loader_seed` | `42` / Inherited from `actor.data_loader_seed` | Random seed for the data loader |
| `critic.cliprange_value` | `0.5` | Clipping range for the Critic value function |
| `critic.loss_agg_mode` | Inherited from `actor.loss_agg_mode` | Loss aggregation mode |
| `critic.grad_clip` | `1.0` | Gradient clipping value |
| `critic.ulysses_sequence_parallel_size` | `1` | Ulysses sequence parallelism size |
| `critic.forward_micro_batch_size` | Inherited from `critic.ppo_micro_batch_size` | Micro-batch size for forward computation |
| `critic.forward_micro_batch_size_per_gpu` | Inherited from `critic.ppo_micro_batch_size_per_gpu` | Micro-batch size per GPU for forward computation |
...
| `critic.model.tokenizer_path` | Inherited from `model.path` | Path to the tokenizer |
| `critic.model.override_config` | `{}` | Overrides the model configuration |
| `critic.model.external_lib` | Inherited from `model.external_lib` | Path to the external library |
| `critic.model.trust_remote_code` | Inherited from `model.trust_remote_code` | Whether to trust remote code |
...
| `algorithm.pf_ppo.reweight_method` | `pow` | Reweighting method of PF-PPO |
| `algorithm.pf_ppo.weight_pow` | `2.0` | Weight power of PF-PPO |
...
| `algorithm.rollout_correction.rollout_is_batch_normalize` | `false` | Whether to batch normalize IS weights |
...
| `trainer.ray_wait_register_center_timeout` | `300` | Ray registration center wait timeout (seconds) |
...
| `actor_rollout_ref.actor.fsdp_config.fsdp_size` | `-1` | FSDP group size; `-1` means global |
...
The configuration structure is the same as that of the Actor FSDP engine, with the main differences being:

| Parameter | Default Value | Description |
|-----------|---------------|-------------|
| `actor_rollout_ref.ref.fsdp_config.forward_only` | `true` | The reference model performs forward computation only |

The remaining parameters (`wrap_policy`, `param_offload`, `optimizer_offload`, `reshard_after_forward`, `fsdp_size`, `dtype`, and so on) have default values consistent with the Actor FSDP engine configuration.
...
The default values of the remaining parameters are the same as those of the Actor FSDP engine configuration.
...
| `actor_rollout_ref.actor.megatron.distrib_optim_fully_reshardable_mem_efficient` | `false` | Whether the distributed optimizer resharding is memory efficient |
...
#### 1.3.7 Model override_config (Megatron Approach)
...
| `profiler.tool` | Inherited from `global_profiler.tool` | Profiler tool. Options: nsys, npu, torch, torch_memory |
| `profiler.all_ranks` | `false` | Whether to enable it on all ranks |
| `profiler.ranks` | `[]` | Specifies the list of ranks to enable |
| `profiler.save_path` | Inherited from `global_profiler.save_path` | Path for saving Profiler results |
...
| `global_profiler.profile_continuous_steps` | `false` | Whether to collect data for continuous steps |
...
| `checkpoint.load_contents` | Referenced from `checkpoint.save_contents` | Contents to load from the checkpoint |
...
The log metrics printed by the reinforcement learning algorithm for each iteration are described as follows:
...
| `actor/pg_clipfrac_lower` | PPO lower-bound clipping proportion (available in some `loss_mode` settings) |
...
| `critic/vf_clipfrac` | Proportion of Critic clipping in effect, reflecting the stability of value function update magnitude |
...
| `critic/vf_explained_var` | Value function explained variance 1 - Var(returns-values)/Var(returns) (printed only when `use_critic=True`) |
...
| `response_length_non_aborted/clip_ratio` | Proportion of non-aborted samples with response lengths reaching the maximum length |
| `response/aborted_ratio` | Proportion of aborted samples (response length of 0) |
...
| Metric | Description |
|------|------|
| `timing_s/gen` | Generation (rollout) time in seconds |
| `timing_s/ref` | Time for the Reference model to compute log_p in seconds |
| `timing_s/values` | Time for the Critic model to compute values in seconds |
| `timing_s/adv` | Time to compute advantage values in seconds |
| `timing_s/update_critic` | Time to update the Critic model in seconds |
| `timing_s/update_actor` | Time to update the Actor model in seconds |
| `timing_s/step` | Total time for one step in seconds |
| `timing_s/old_log_prob` | Time for the Actor model to compute old log_p in seconds |
| `timing_s/reward` | Time for reward computation in seconds |
| `timing_s/testing` | Time for validation in seconds |
| `timing_s/save_checkpoint` | Time to save a checkpoint in seconds |
| `timing_s/update_weights` | Time for weight synchronization in seconds |
| `timing_per_token_ms/gen` | Time per token during the generation phase in milliseconds |
| `timing_per_token_ms/ref` | Time per token for the Reference model in milliseconds |
| `timing_per_token_ms/values` | Time per token for the Critic model in milliseconds |
| `timing_per_token_ms/adv` | Time per token for advantage computation in milliseconds |
| `timing_per_token_ms/update_critic` | Time per token for Critic updates in milliseconds |
| `timing_per_token_ms/update_actor` | Time per token for Actor updates in milliseconds |

### 2.6 Performance Metrics

| Metric | Description |
|------|------|
| `perf/total_num_tokens` | The total number of tokens processed in this step |
| `perf/time_per_step` | The total time spent in this step (seconds) |
| `perf/throughput` | Throughput: tokens / (time * n_gpus) |
| `perf/max_memory_allocated_gb` | The maximum allocated GPU memory (GB) |
| `perf/max_memory_reserved_gb` | The maximum reserved GPU memory (GB) |
| `perf/cpu_memory_used_gb` | The used CPU memory (GB) |
| `perf/mfu/actor` | The MFU (model floating-point utilization) for Actor training |
| `perf/mfu/critic` | The MFU for Critic training |
| `perf/mfu/actor_infer` | The MFU for the Actor inference phase |

### 2.7 Variance Proxy Metrics

| Metric | Description |
|------|------|
| `variance_proxy/proxy1_signal_strength` | Signal strength: the squared norm of the mean gradient \|\|g_mean\|\|^2 |
| `variance_proxy/proxy2_total_power` | Total power: the expectation of the squared norm of the gradient E[\|\|g_tau\|\|^2] |
| `variance_proxy/proxy3_pure_noise` | Pure noise: the gradient variance proxy (1/(N-1)) * (Proxy2 - Proxy1) |
| `variance_proxy/expected_a_squared` | The expectation of the squared advantage E[A^2] |
| `variance_proxy/expected_w` | The expectation of the W-score proxy E[W] |
...
| `global_seqlen/minmax_diff` | The difference between max and min before balancing |
...
When using only the GDPO estimator, print:

| Metric | Description |
|--------|-------------|
| `gdpo/{key}/mean` | The mean of each reward component in GDPO |
| `gdpo/{key}/std` | The standard deviation of each reward component in GDPO |
| `gdpo/{key}/max` | The maximum value of each reward component in GDPO |
| `gdpo/{key}/min` | The minimum value of each reward component in GDPO |
