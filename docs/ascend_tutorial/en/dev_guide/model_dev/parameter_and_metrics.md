...
| `actor_rollout_ref.actor.optim.total_training_steps` | `-1` | Total number of training steps; `-1` means it is calculated automatically |
| `actor_rollout_ref.actor.optim.weight_decay` | `0.01` | Weight decay, used to prevent model overfitting |
| `actor_rollout_ref.actor.optim.lr_warmup_steps` | `-1` | Number of learning rate warmup steps; `-1` means it is calculated automatically from the ratio |
| `actor_rollout_ref.actor.optim.betas` | `[0.9, 0.999]` | First and second moment coefficients for the Adam optimizer |
...
| `actor_rollout_ref.actor.entropy_coeff` | `0` | Entropy regularization coefficient that controls the degree of policy exploration. |
...
| `actor_rollout_ref.actor.entropy_from_logits_with_chunking` | `false` | Whether to calculate entropy from logits using a chunking method. |
...
| `actor_rollout_ref.rollout.enforce_eager` | `false` | Whether to force PyTorch eager mode |
...
| `actor_rollout_ref.rollout.calculate_log_probs` | `false` | Whether to compute log probs during the rollout phase |
...
|--------|--------|------|
| `actor_rollout_ref.rollout.multi_turn.enable` | `false` | Whether to enable multi-turn dialogue |
...
| `actor_rollout_ref.rollout.multi_turn.tool_response_truncate_side` | `middle` | Side from which tool responses are truncated |
...
| `actor_rollout_ref.rollout.multi_turn.format` | `hermes` | Multi-turn dialogue format |
...
| Parameter | Default Value | Description |
|--------|--------|------|
| `actor_rollout_ref.ref.rollout_n` | Referenced from `rollout.n` | Number of rollout iterations |
...
| `actor_rollout_ref.ref.log_prob_use_dynamic_bsz` | Referenced from `actor.use_dynamic_bsz` | Whether log prob uses dynamic batch size |
| `actor_rollout_ref.ref.log_prob_max_token_len_per_gpu` | Referenced from `actor.ppo_max_token_len_per_gpu` | Maximum token length for log prob per GPU |
| `actor_rollout_ref.ref.ulysses_sequence_parallel_size` | Referenced from `actor.ulysses_sequence_parallel_size` | Ulysses sequence parallel size |
...
| Parameter | Default Value | Description |
|-----------|----------------|-------------|
| `critic.optim.lr` | `1.0e-05` | Learning rate for the critic. |
| `critic.optim.lr_warmup_steps_ratio` | `0.0` | Ratio of warmup steps for the learning rate. |
| `critic.optim.total_training_steps` | `-1` | Total number of training steps. |
| `critic.optim.weight_decay` | `0.01` | Weight decay. |
| `critic.optim.lr_warmup_steps` | `-1` | Number of warmup steps for the learning rate. |
| `critic.optim.betas` | `[0.9, 0.999]` | Momentum coefficients for the Adam optimizer. |
| `critic.optim.clip_grad` | `1.0` | Gradient clipping threshold. |
| `critic.optim.override_optimizer_config` | `null` / `{}` | Overrides the optimizer configuration. |
...
| Parameter | Default Value | Description |
|--------|--------|------|
| `trainer.balance_batch` | `true` | Whether to balance batches |
| `trainer.total_epochs` | `30` | Total number of training epochs |
| `trainer.total_training_steps` | `null` | Total number of training steps; `null` means it is automatically calculated from epochs |
...
| `trainer.save_freq` | `-1` | Saving frequency; `-1` means no saving |
...
| `trainer.rollout_data_dir` | `null` | Address configuration for saving rollout results from each round |
...
| `actor_rollout_ref.model.lora_rank` | `0` | LoRA rank, where 0 means LoRA is not used |
...
The configuration structure is the same as that of the Actor FSDP engine, with the main differences as follows:
...
All other parameters use the same default values as the Actor FSDP engine configuration.
...
| `actor_rollout_ref.actor.optim.lr_decay_style` | `constant` | Learning rate decay style, such as constant, cosine, exponential, and so on |
...
| `profiler.save_path` | Referenced from `global_profiler.save_path` | Path to save Profiler results |
...
| `checkpoint.load_contents` | References `checkpoint.save_contents` | Contents to load from the checkpoint |
...
| `critic/vf_clipfrac` | Proportion of Critic clipping mechanism activations, reflecting the stability of value function update magnitude |
...
| `response_length/clip_ratio` | Proportion of responses reaching the maximum length |
...
| `response_length_non_aborted/clip_ratio` | Proportion of non-aborted samples reaching the maximum response length |
| `response/aborted_ratio` | Proportion of aborted samples (with a response length of 0) |
| `prompt_length/mean` | Mean prompt length |
| `prompt_length/max` | Maximum prompt length |
| `prompt_length/min` | Minimum prompt length |
| `prompt_length/clip_ratio` | Proportion of prompts reaching the maximum length |
...
| `tool_call_counts/mean` | Mean number of tool calls (printed only when `tool_call_counts` is present) |
...
| `timing_s/ref` | Time spent on computing log_p by the Reference model in seconds |
| `timing_s/values` | Time spent on computing values by the Critic model in seconds |
| `timing_s/adv` | Time spent on computing advantage values in seconds |
| `timing_s/update_critic` | Time spent on updating the Critic model in seconds |
| `timing_s/update_actor` | Time spent on updating the Actor model in seconds |
| `timing_s/step` | Total time spent on one step in seconds |
| `timing_s/old_log_prob` | Time spent on computing old log_p by the Actor model in seconds |
| `timing_s/reward` | Time spent on computing rewards in seconds |
| `timing_s/testing` | Time spent on validation in seconds |
| `timing_s/save_checkpoint` | Time spent on saving checkpoints in seconds |
| `timing_s/update_weights` | Time spent on weight synchronization in seconds |
| `timing_per_token_ms/gen` | Time spent per token in the generation phase in milliseconds |
...
| `timing_per_token_ms/adv` | Time spent per token on advantage computation in milliseconds |
| `timing_per_token_ms/update_critic` | Time spent per token on Critic updates in milliseconds |
| `timing_per_token_ms/update_actor` | Time spent per token on Actor updates in milliseconds |
...
| `perf/time_per_step` | The total time consumed in this step (seconds) |
| `perf/throughput` | Throughput: tokens / (time * n_gpus) |
| `perf/max_memory_allocated_gb` | The maximum allocated memory on the GPU (GB) |
| `perf/max_memory_reserved_gb` | The maximum reserved memory on the GPU (GB) |
| `perf/cpu_memory_used_gb` | The memory used on the CPU (GB) |
...
| `global_seqlen/mean` | The average sum of sequence lengths across partitions |

#### 2.8.3 GDPO Reward Metrics

Print only when using the GDPO estimator:
