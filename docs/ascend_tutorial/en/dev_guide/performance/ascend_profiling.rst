...
- Role profile control: Control collection parameters and other settings through the configuration items in each role.
...
- **save_path**: The path to save the collected data. The default value is "outputs/profile".
...
-  ranks: The list of ranks from which to collect data. If empty, no data is collected.
...
-  contents: Options for controlling the collection content, for example,
         ...
         actor:  # Set the profiler collection configuration parameters for the actor role
            profiler:
               ...
               tool_config:
                  npu:
                     discrete: True
                     contents: [npu, cpu]  # Controls the collection list; the default is cpu and npu, and you can configure memory, shapes, module, and so on
      ...
      actor_rollout_ref:
         ...
         rollout:
            profiler:
               ...
               all_ranks: False
               ranks: [0]  # Global GPU rank; it is mapped to the inference replica that owns this rank
               tool_config:
                  npu:
...
global_profiler.tool=npu
        ...
        actor_rollout_ref.actor.profiler.tool_config.npu.discrete=True # Discrete mode is recommended; data from each stage is stored separately
...
In `Agent Loop <../../../../advance/agent_loop.rst>`_ mode, performance data from the Rollout phase **must be collected using discrete mode**, where the Profiler is triggered by the inference engine backend.
...
2. Inference engine support: The vLLM and SGLang engines are currently supported, with no additional configuration required. The details are as follows:
...
1. In `Fully Async Policy <https://verl.readthedocs.io/en/latest/advance/fully_async.html>`_ mode, `global_profiler.steps` represents the `step` after each `update_weights` call, which is consistent with synchronous mode, rather than the `mini-batch step` of a single round.

2. Because this reuses the AgentLoop collection capability, the precautions in `Fully Async Policy <https://verl.readthedocs.io/en/latest/advance/fully_async.html>`_ mode are the same as those for AgentLoop.

Visualization
---------------

The collected data is stored in the `save_path` that you set. You can visualize it using the `MindStudio Insight <https://www.hiascend.com/document/detail/zh/mindstudio/80RC1/GUI_baseddevelopmenttool/msascendinsightug/Insight_userguide_0002.html>`_ tool.
...
- Seamless integration: You can run the MindStudio Insight tool directly in a Jupyter environment without switching platforms or copying data from the server, enabling immediate data use.
- Quick startup: You can quickly start the MindStudio Insight tool through the JupyterLab command line or graphical interface.
...
- Remote access: You can start MindStudio Insight remotely and connect to the service from a local browser for direct visual analysis, which alleviates the difficulties of uploading and downloading data for large model training or inference.

If the `analysis` parameter is set to `False`, offline parsing is required after collection.
...
1. Add a script to control the profiling granularity
~~~~~~~~~~~~~~~~~~~~~~~~~
...
2. Fine-grained collection during the Rollout phase
~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~

For the vLLM or SGLang inference engine, you can control the ``schedule`` parameter to collect the forward propagation performance data of the model at a specific token.
...
if os.environ.get('ROLLOUT_PROFILE', "false") == "true":
    ...
    self.profiler_npu = torch_npu.profiler.profile(
        ...
        on_trace_ready=torch_npu.profiler.tensorboard_trace_handler(os.environ.get('ROLLOUT_PROFILE_PATH'), analyse_flag=True)  # Path to save collected data, whether to parse online
    )
...
3. Fine-grained profiling of the update_policy (Actor & Critic) phase
~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~
...
- **Modified files**: ``verl/workers/engine_workers.py``
...
for batch_idx, mini_batch_td in enumerate(dataloader):
    # ... Internally calls self.train_batch(mini_batch_td), which executes
    # Forward & Backward for each micro-batch inside the engine and completes
    # one optimizer update ...
    actor_output = self.train_batch(mini_batch_td)

+              if self.step == int(os.environ.get('PROFILE_STEP', 1)) and os.environ.get('UPDATE_PROFILE', "false") == "true":
+                  # Drive the schedule to profile the mini batch. To profile a micro batch, move self.prof_npu.step() into the micro batch loop.
+                  if str(torch.distributed.get_rank()) in os.environ.get('PROFILE_RANKS', "0").split(','):
+                      self.prof_npu.step()
...
This stage computes the probability distributions of the new and old policies. Under the unified model engine, the log-prob calculations for both the actor and the ref go through ``TrainingWorker.infer_batch``, which ultimately dispatches to the ``BaseEngine.infer_batch`` of the corresponding backend engine.

**FSDP backend**

FSDP backend allows fine-grained control at the micro-batch level, enabling instrumentation within the micro-batch loop of the FSDP engine's forward pass.

- **File to modify**: ``verl/workers/engine/fsdp/transformer_impl.py``
...
+           # Prepare the profiler (configuration as above, omitted for brevity)
+           experimental_config = torch_npu.profiler._ExperimentalConfig(...)
+           self.prof_npu = torch_npu.profiler.profile(
+               # ...  (configuration as above, omitted for brevity)
+               # wait=0, warmup=0, active=1: directly capture the first micro-batch
+               schedule=torch_npu.profiler.schedule(wait=0, warmup=0, active=1, repeat=1),
...
+                   # Collect the micro batch for the driver schedule
+                   if role == "Ref":
+                       self.prof_npu.step()
...
The Micro-Batch scheduling for the Megatron backend is managed internally by Megatron's pipeline parallel ``forward_backward_func``. Fine-grained Micro-Batch-level profiling through simple code instrumentation is not currently supported. We recommend using the global profiler configuration for profiling.
