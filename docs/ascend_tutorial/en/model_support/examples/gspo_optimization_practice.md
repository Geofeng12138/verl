...
To ensure a smooth experience, switch verl to the `main` branch, with commit ID `9d05508f5e3bd8ecb70cf94ab10dc087b57a716d`. Note: the `main` branch may cause patch issues due to iterative refactoring. If you need a stable version, switch to `release/v0.8.0`.
...
GSPO improves training stability by raising the optimization granularity from the **token level** to the **sequence level**, which avoids the **sharp increase in variance** that GRPO encounters and that leads to unstable training. This algorithm also improves the convergence speed to a certain extent.

To successfully use the GSPO algorithm in the verl repository, you need to complete the following required configuration.
...
If the methods above do not work, you can install it by compiling from source. Go to the jemalloc official website to download the latest stable version. The website address is: https://github.com/jemalloc/jemalloc/releases/
...
Before running the task, run the following command to import jemalloc through environment variables:
...
For the multi-machine tasks provided in this practice, use the following script to launch them.
...
# Modify this to the path of the test case you need to run
DEFAULT_SH="./run_*.sh"
echo "Use $DEFAULT_SH"
...
NNODES=4
NPUS_PER_NODE=16
# Modify this to the IP address of the corresponding master node
...
SOCKET_IFNAME, HCCL_SOCKET_IFNAME, GLOO_SOCKET_IFNAME: Change them to the corresponding communication network interface. You can obtain the communication network interface by running the following command:
...
**This optimization mainly adjusts the two parameters above. However, note that setting these parameters too large can cause OOM.**

**Key adjustments** Increasing `actor_ppo_max_token_len` reduces training time. Adjusting `infer_ppo_max_token_len` provides no significant benefit, so you can leave it unchanged.

**The following describes the purpose of these two parameters:**

**These two parameters control the maximum number of tokens processed by each GPU in dynamic batch size mode.**

- **`actor_ppo_max_token_len`**: The maximum number of tokens that each GPU can process when the Actor model performs PPO updates (forward and backward propagation).
...
For inference operator dispatch optimization, you can typically achieve a performance gain of about `15%~20%`.
...
The default generated algorithm is as follows, for your reference.
...
Keeping it enabled is generally beneficial.
...
- AI_CPU: Indicates that the orchestration expansion location of the communication algorithm is on the AI CPU computing unit on the Device side.
- AIV: Indicates that the orchestration expansion location of the communication algorithm is on the Vector Core computing unit on the Device side.
...
- The software stack runs on the host CPU, where the communication algorithm expands into individual tasks.
...
- STARS schedules kernels on the kernel queue stream and places them on the AiCPU for execution.
...
- This reduces host-AiCPU interaction from hundreds of times to once.
...
TASK_QUEUE_ENABLE, dispatch optimization, set graph mode to 1 (that is, set it to 1 when graph mode is enabled), and set it to 2 when graph mode is not used.
...
Here are some memory-related settings and switches. Note that the optimizations in this section may, to varying degrees, degrade throughput.

```bash
# Gradient Checkpointing
# Purpose: Saves device memory by recomputing activations, trading computation for memory. Intermediate activations are not saved during the forward pass and are recomputed during the backward pass, which significantly reduces memory usage and allows for a larger batch size.
...
# Free Cache Engine
# Purpose: Releases the inference engine's KV cache and weights during the training phase. This is a core optimization of the 3D-HybridEngine, allowing inference and training to alternate on the same GPU and significantly reducing device memory requirements.
actor_rollout_ref.rollout.free_cache_engine=True \

# Entropy Computation Optimization
# entropy_checkpointing: Enables recomputation for entropy computation during training to reduce peak memory usage.
# entropy_from_logits_with_chunking: Processes logits tensors in chunks (for example, 2048 tokens per group) to avoid loading the entire [bsz*seq_len, vocab] tensor at once.
actor_rollout_ref.actor.entropy_checkpointing=True \
actor_rollout_ref.ref.entropy_checkpointing=True \
actor_rollout_ref.actor.entropy_from_logits_with_chunking=True \
actor_rollout_ref.ref.entropy_from_logits_with_chunking=True \

# Inference Engine Memory Configuration
# gpu_memory_utilization: Controls the proportion of GPU memory used by vLLM (0.90 = 90%).
# enforce_eager=False: Enables CUDA graphs to accelerate inference, but this uses additional device memory.
actor_rollout_ref.rollout.gpu_memory_utilization=0.90 \
actor_rollout_ref.rollout.enforce_eager=False \
```

## NPU Optimization Reference Articles
