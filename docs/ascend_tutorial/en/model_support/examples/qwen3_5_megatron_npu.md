...
This guide describes how to run the Qwen3.5-35B-A3B and Qwen3.5-122B-A10B GRPO examples on Ascend NPUs using verl + Megatron + vLLM.
...
The example script uses the following NPU configuration by default, which can be overridden through environment variables with the same names:
...
Before starting training, start the Ray cluster. For general multi-node instructions, refer to [Multinode Training](../../../../start/multinode.rst). For an Ascend multi-node script example, refer to [Ascend SGLang Best Practices](ascend_sglang_best_practices.rst).

The minimal startup method is as follows: for a single-machine task, run only the head node command; for a multi-machine task, run the worker node command on the other nodes. `MASTER_ADDR` must be consistent across all nodes, and set `CURRENT_IP` to the current node's IP address.
