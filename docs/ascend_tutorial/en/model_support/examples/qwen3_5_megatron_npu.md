# Qwen3.5 Megatron NPU Usage Guide

Last updated: 09/07/2026.

This guide provides instructions for running the Qwen3.5-35B-A3B and Qwen3.5-122B-A10B GRPO examples using verl + Megatron + vLLM on Ascend NPUs.
...
We recommend using the image listed in the table above directly.
...
The sample script uses the following NPU configuration by default, which you can override with environment variables of the same name:
...
Model weights can be specified using the Hugging Face model name, or you can download them to a local path in advance:
...
Before training, start the Ray cluster. For general multi-node instructions, refer to [Multinode Training](../../../../start/multinode.rst). For an Ascend multi-node script example, refer to [Ascend SGLang Best Practices](ascend_sglang_best_practices.rst).

The minimal startup method is as follows. For single-machine tasks, you only need to run the head node command; for multi-machine tasks, you need to run the worker node command on other nodes. `MASTER_ADDR` must be consistent across all nodes, and set `CURRENT_IP` to the current node's IP address.
...
After confirming through `ray status` that the number of NPU resources meets expectations, run the training script on the main node. Qwen3.5-35B-A3B requires 16 NPU resources by default, and Qwen3.5-122B-A10B requires 64 NPU resources by default.
...
- The script automatically detects the NPU environment through `torch_npu`; to specify it manually, set `DEVICE=npu`.
- Qwen3.5's Gated Delta Net currently does not use packed sequences, so the script keeps `use_remove_padding=False` and `use_dynamic_bsz=False`.
- The NPU branch sets Ascend adaptation parameters such as `vanilla_mbridge=False`, `use_flash_attn=True`, and `moe_token_dispatcher_type=alltoall`.
