...
vLLM is currently a mainstream high-performance open-source inference engine, and Ascend has fully provided native support for using this inference engine in verl. With a simple build process, developers can complete the environment setup. This article provides two classic use cases to help developers understand the following:

1. Environment Setup
2. Model Training and Evaluation
3. Performance Profiling

The use case model script and the required hardware conditions are as follows:
...
+----------------------+---------------------+----------+----------------------------+
| Model                | NPU model           | Number of nodes | Training and inference backend |
+======================+=====================+==========+============================+
...
For single-machine tasks with `Qwen3-30B`_, you can directly run the example script from the verl repository using bash, for example:
...
The steps are the same for different models. The Qwen3-30B model is used as an example.
...
The content of the Python file is as follows. `host_port` must match the `port` on the server side. Modify `max_seq_len` and `max_out_len` according to the model configuration. The inference example is set to 2k input and 20k output.
...
data-parallel-size: The number of data parallel replicas. It is recommended that the DP configuration remain consistent with the inference configuration used during training. The default value is 1.
