...
vLLM is a mainstream high-performance open-source inference engine, and Ascend provides full native support for using it with verl. With a simple build process, developers can set up the environment. This article provides two classic use cases to help developers understand the following:
...
# Save frequency. -1 means do not save by default. Modify this parameter if you need to evaluate.
...
# Modify to the path of the test case you need to run
DEFAULT_SH="./run_*.sh"
echo "Use $DEFAULT_SH"
...
The content of the Python file is as follows. `host_port` must match the `port` on the server side. Modify `max_seq_len` and `max_out_len` based on the model configuration. In this inference example, they are set to 2k input and 20k output:
...
port: You can set it to any available port.
...
Note: Collecting full Profiling on the verl framework side generates a large number of duplicate operator records. You can modify the code according to the documentation to collect only key stages.
