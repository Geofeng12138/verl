...
* **Dynamic Sampling**: Improves training efficiency and stability. DAPO proposes a strategy that performs dynamic sampling and filters out prompt groups with accuracy equal to 1 and 0, thereby keeping the number of prompts with effective gradients consistent across batches.
* **Token-level Policy Gradient Loss**: Is critical in long-CoT reinforcement learning scenarios.
...
- **Dynamic sampling configuration**
  Setting `filter_groups.enable` to `True` filters out groups whose output `metric` is exactly the same. For example, for the `acc` metric, groups whose output accuracy is all 1 or 0 are filtered out.
  The trainer uses `gen_batch_size` for repeated sampling until enough eligible groups are generated or the upper limit specified by `max_num_gen_batches` is reached.
...
- **Token-level Loss**
  Setting `loss_agg_mode` to `token-mean` means that the average of the (policy gradient) loss is calculated for all tokens in all sequences within a batch.
...
- **Reward model penalty configuration for overlong responses**
  Setting `overlong_buffer.enable` to `True` penalizes outputs that are too long but still within the hard context limit. Specifically, when the output length exceeds `max_response_length - overlong_buffer.len` and is between `0` and `overlong_buffer.len` tokens beyond that threshold, the penalty value increases linearly from `0` to `overlong_buffer.penalty_factor`.
...
reward_model.overlong_buffer.len=${overlong_buffer_len}  # Buffer length, defines the buffer tokens and maximum penalty strength
...
The Geometry3k dataset is a geometry-specific dataset developed jointly by the University of California, Los Angeles and Zhejiang University. It focuses on visual question answering (VQA) tasks for research and model training. The dataset contains 3,002 samples in total, built with two modalities of data: image and text. The text modality covers descriptions of various geometry problems, while the images present the geometric figure information in the problems as visual diagrams, including basic shapes such as triangles, circles, and quadrilaterals, as well as relationships between different figures, such as position, nesting, and intersection. You can download the original dataset from the Hugging Face library: [Geometry3k](https://huggingface.co/datasets/hiyouga/geometry3k)
...
Install jemalloc from the operating system source (note: Ubuntu version >= 20.04 is required):
...
- Some models are optimized through vLLM Ascend. However, in certain cases, the optimized model may not be applicable. In such cases, set this value to 0 to disable the optimized model.
...
- Fallback configuration for Ascend multi-device communication, extending the connection timeout to prevent training startup failures in cluster environments caused by slow connections.
...
```
- For multi-node tasks with Qwen3-VL-30B, we recommend using the following script to launch large-scale multi-node training. Modify `NNODES` and `NPUS_PER_NODE` according to your actual needs, and update the parameters `trainer.nnodes` and `trainer.n_gpus_per_node` in the configuration script to match.
...
MASTER_ADDR="IP FOR MASTER NODE"
# Modify to the communication network interface of the current node
...
NNODES and NPUS_PER_NODE: Set these to the number of nodes and the number of NPUs per node. In this case, they are 2 and 8, respectively.

MASTER_ADDR: Change this to the IP address of the corresponding master node. That is, the MASTER_ADDR on all nodes should be the same.
...
- **Maximum number of tokens that a single GPU can process**
...
- **Single GPU micro-batch size**
  When `use_dynamic_bsz=True`, the framework uses this value as the initial batch size, and then adjusts it upward or downward based on `ppo_max_token_len_per_gpu`.
...
- **Enable the FSDP2 framework**
  "Store model parameters, gradients, and optimizer states sharded across different GPUs" to avoid device memory overflow caused by loading the full model on a single device.
