...
* **Clip-Higher**: It promotes system diversity and avoids entropy collapse by clipping the upper bound of the importance sampling ratio.
* **Dynamic Sampling**: It improves training efficiency and stability. DAPO proposes a strategy that performs dynamic sampling and filters out prompt groups with accuracy equal to 1 and 0, thereby keeping the number of prompts with effective gradients consistent across batches.
* **Token-level Policy Gradient Loss**: It is crucial in long-CoT RL scenarios.
* **Overlong Reward Shaping**: It reduces reward noise and stabilizes training.
...
```bash
clip_ratio_low=0.2  # Lower bound of the clipping ratio, default value is 0.2
clip_ratio_high=0.28 # Upper bound of the clipping ratio, default value is 0.28
```

- **Dynamic sampling configuration**
  Setting `filter_groups.enable` to `True` filters out groups whose output `metric` values are identical. For example, for the `acc` metric, it filters out groups whose output accuracy is all 1 or 0.
  The trainer uses `gen_batch_size` for repeated sampling until it generates a sufficient number of eligible groups or reaches the upper limit specified by `max_num_gen_batches`.
...
algorithm.filter_groups.max_num_gen_batches=${max_num_gen_batches} # Maximum number of generation batches, that is, the maximum number of times to repeat data generation
```
...
- **Reward model penalty configuration for overly long responses**
  Setting `overlong_buffer.enable` to `True` penalizes outputs that exceed the desired length but do not yet surpass the hard context limit. Specifically, when the output length exceeds `max_response_length - overlong_buffer.len` and is between `0` and `overlong_buffer.len` tokens beyond that threshold, the penalty value increases linearly from `0` to `overlong_buffer.penalty_factor`.
...
reward_model.overlong_buffer.len=${overlong_buffer_len}  # Buffer length, defining the buffer tokens and maximum penalty strength
reward_model.overlong_buffer.penalty_factor=${overlong_penalty_factor}   # Penalty factor, maximum penalty strength
```

For related parameter code, refer to: [Recipe: Decoupled Clip and Dynamic Sampling Policy Optimization (DAPO)](https://github.com/verl-project/verl-recipe/blob/main/dapo/README.md)

## Hardware Requirements


Currently supported: Atlas 800T A3 and Atlas 900 A3 SuperPoD. Completing this best practice requires one Atlas 900 A3 SuperPoD. For key software versions, refer to the [Ascend Quick Start Guide](https://github.com/verl-project/verl/blob/main/docs/ascend_tutorial/zh/get_start/quick_start.rst).
...
The Geometry3k dataset is a geometry-specific dataset developed jointly by the University of California, Los Angeles and Zhejiang University. It focuses on research and model training for visual question answering (VQA) tasks. The dataset contains 3,002 samples in total and is constructed with two modalities of data: image and text. The text modality covers descriptions of various geometry problems, while the images present the geometric information in the problems as visual diagrams, including basic shapes such as triangles, circles, and quadrilaterals, as well as relationships between different shapes, such as position, nesting, and intersection. You can download the original dataset from Hugging Face: [Geometry3k](https://huggingface.co/datasets/hiyouga/geometry3k)
...
Install jemalloc through the operating system source (note: Ubuntu version 20.04 or later is required):
...
If the methods above fail to install properly, you can install it by compiling from source. Go to the jemalloc official website to download the latest stable version. The website address is: https://github.com/jemalloc/jemalloc/releases/
...
- Some models are optimized through vLLM Ascend. However, in some cases, the optimized model may not be applicable. In such cases, set this value to 0 to disable the optimized model.
...
- Fallback configuration for Ascend multi-device communication, extending the connection timeout to prevent training startup failures caused by slow connections in a cluster environment.
...
- For single-machine tasks with Qwen3-VL-30B, use the following script to start training.
...
# Modify to the IP address of the master node
...
```
- For multi-node tasks with Qwen3-VL-30B, we recommend using the following script to launch large-scale multi-node training. Modify `NNODES` and `NPUS_PER_NODE` as needed, and update the parameters `trainer.nnodes` and `trainer.n_gpus_per_node` in the configuration script to match them.
...
# Modify to the path of the test case to run
DEFAULT_SH="./run_*.sh"
echo "Use $DEFAULT_SH"
...
# Modify to the IP address of the master node
MASTER_ADDR="IP FOR MASTER NODE"
# Modify to the communication NIC of the current node
...
sleep 600
```
DEFAULT_SH: Modify it to the path of the sh file for the training configuration. In this case, modify it to the path of [Qwen3_VL_30B](https://github.com/verl-project/verl-recipe/blob/main/dapo/run_dapo_qwen3_vl_30b_fsdp2_npu.sh).
...
- **Maximum number of tokens a single GPU can process**
...
- **Enable the FSDP2 framework**
  "Shard model parameters, gradients, and optimizer states across different GPUs" to avoid device memory overflow caused by loading the full model on a single device.
...
# For FSDP2 only: reshard after forward pass to reduce memory usage.
actor_rollout_ref.actor.fsdp_config.reshard_after_forward=True
# For FSDP2 only: whether to reshard after the model forward pass to save memory.
