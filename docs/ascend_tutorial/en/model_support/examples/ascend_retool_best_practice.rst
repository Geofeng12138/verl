...
ReTool paper reference ([ReTool](https://arxiv.org/pdf/2504.11536))
Integrate a code interpreter tool, deploy the policy through multi-turn real-time code execution, and teach the model to learn when and how to call tools based on result feedback.

1. Environment Setup
2. Model Training

The use case model script and the hardware conditions it requires are as follows:
...
*Note: ReTool-SFT is downloaded automatically, and the generated data is saved in the ~/ReTool-SFT/data directory by default.*

**Run the pretraining script**

.. code-block:: bash

bash recipe/retool/run_qwen2_7b_sft_npu.sh # Adjust the paths in the script as needed

**Merge pretrained weights to generate a checkpoint**
...
An example configuration file is provided below. Create a `run_qwen2.5_7b_dapo_npu.sh` file in the `recipe/retool` directory.
Modify the following parameters in the model training script based on the actual paths configured by the developer.
