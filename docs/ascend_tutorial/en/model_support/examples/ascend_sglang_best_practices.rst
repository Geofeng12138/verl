...
SGLang is currently a mainstream high-performance open-source inference engine, and Ascend has fully provided native support for using this inference engine in verl. With a simple build process, developers can complete the environment setup. This guide helps developers understand the following:

1. Environment Setup
2. Model Training and Evaluation
3. Performance Profiling

The use case model scripts and the required hardware conditions are as follows:

- Note: verl recently underwent script cleanup and naming changes. It is recommended to build based on the documentation corresponding to commit id c98cb8cc, `doclink`_, and the associated scripts to avoid broken links.
...
*Note: verl currently supports mbridge for flexible weight conversion between HF and mcore. You can modify the relevant parameters below to directly load HF weights.*
...
# Save frequency. -1 means no saving by default. If you need to evaluate, modify this parameter.
trainer.save_freq=-1

For single-machine tasks with `Qwen3-30B`_, you can directly run the example script from the verl repository using bash.
...
# TASK_QUEUE_ENABLE, dispatch optimization; set to 1 for graph mode, 2 for non-graph mode
...
# Check whether device_count equals NNODES
    ...
    # Child node attempts to register with the master node until successful
    while true; do
...
MASTER_ADDR: Set this to the IP address of the corresponding master node. That is, the MASTER_ADDR of all nodes should be the same.
...
We use AISBenchmark to evaluate models. This tool supports evaluation of multiple inference backends, including vLLM and SGLang.
...
models = [
    dict(
        ...
        path="/path/to/Qwen3-30B", # Change to the path of the Qwen3-30B model
        model="qwen3-30b",
        request_rate = 0,
        max_seq_len=2048,
        retry = 2,
