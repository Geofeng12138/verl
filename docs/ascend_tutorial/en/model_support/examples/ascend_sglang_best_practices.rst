...
SGLang is currently a mainstream high-performance open-source inference engine. Ascend NPU provides full native support for this inference engine in verl. With a simple build process, you can complete the environment setup. This guide helps you understand the following:
...
- Note: verl has recently performed script cleanup and renaming. To avoid broken links, build using the documentation `doclink`_ and the corresponding scripts for commit id c98cb8cc.

+----------------------+---------------------+----------+------------------------+
| Model                | NPU Model           | Nodes    | Train/Infer Backend    |
+======================+=====================+==========+========================+
...
Environment Setup
-----------------------------------
The `Ascend Installation Guide <../../get_start/install_guidance.rst>`_ provides two methods for building the environment: 1. Build from a Dockerfile image 2. Build from a custom Conda environment
...
--local-dir: The path to save the model
...
For the single-node task `Qwen3-30B`_, you can directly run the sample script from the verl repository using bash.
...
pkill -9 python
  ...
  # TASK_QUEUE_ENABLE: dispatch optimization. Set to 1 for graph mode, and set to 2 for non-graph mode
...
export HCCL_HOST_SOCKET_PORT_RANGE=60000-60050
  ...
  # Modify to the path of the use case you need to run
  DEFAULT_SH="./run_*.sh"
  echo "Use $DEFAULT_SH"
...
MASTER_ADDR: Change this to the IP address of the master node. That is, the MASTER_ADDR of all nodes should be the same.
...
The steps are the same for different models. This guide uses Qwen3-30b as an example.

We evaluate the model using AISBenchmark. This tool supports evaluating multiple inference backends, such as vllm and sglang.
...
After training, the model's score on Math-500 improves significantly.
...
After data collection is complete, you can use `MindStudio Insight <https://www.hiascend.com/document/detail/zh/mindstudio/830/GUI_baseddevelopmenttool/msascendinsightug/Insight_userguide_0002.html>`_ to analyze the data.
