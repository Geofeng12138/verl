...
SGLang is a mainstream high-performance open-source inference engine. Ascend now provides full native support for using this inference engine in verl. With a simple build process, developers can complete the environment setup. This guide helps developers understand the following:
...
- Note: verl recently underwent script cleanup and naming changes. It is recommended to build based on the documentation corresponding to commit id c98cb8cc, as referenced in `doclink`_ , and the corresponding scripts to avoid broken links.

+----------------------+---------------------+----------+------------------------+
| Model                | NPU Model           | Number of Nodes | Training/Inference Backend |
+======================+=====================+==========+========================+
...
Environment Setup
-----------------------------------
We provide two methods for setting up the environment in the `Ascend Installation Guide <../../get_start/install_guidance.rst>`_: 1. Build from the Dockerfile image file 2. Build from a custom Conda environment
...
--local-dir: the path for saving the model
...
2. Training
^^^^^^^^^^^
Modify the following parameters in the model training script based on the developer's actual path configuration.
...
# Save frequency. The default value -1 means that checkpoints are not saved. To enable evaluation, modify this parameter.
trainer.save_freq=-1

For single-machine tasks `Qwen3-30B`_, you can directly run the example script from the verl repository using bash.

.. code-block:: bash 

bash examples/grpo_trainer/run_qwen3moe-30b_sglang_megatron_npu.sh
If you want to scale up to multiple nodes, we recommend using the following script to launch large-scale multi-node training.
...
# TASK_QUEUE_ENABLE, optimization for task dispatch; set to 1 for graph mode, 2 for non-graph mode
...
# Change to the path of the test case you want to run
DEFAULT_SH="./run_*.sh"
echo "Use $DEFAULT_SH"
...
The steps are the same across different models; Qwen3-30b is used as an example here.
...
Open the `benchmark/ais_bench/benchmark/configs/models/vllm_api/vllm_api_stream_chat.py` file, which is the inference configuration file.
...
**Start the sglang_server service**
...
In the `Qwen3-30B`_ script, the basic profiling option `PROF_CONFIG` is provided. By default, `global_profiler.steps=null` disables profiling. You can modify the parameters based on your actual needs.

After collection is complete, developers can use `MindStudio Insight <https://www.hiascend.com/document/detail/zh/mindstudio/830/GUI_baseddevelopmenttool/msascendinsightug/Insight_userguide_0002.html>`_ to parse the data.

Note: Collecting full Profiling on the verl framework side generates a massive number of duplicate operator records. You can modify the code according to the documentation to collect only the key stages.