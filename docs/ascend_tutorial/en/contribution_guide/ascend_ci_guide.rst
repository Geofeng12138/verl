...
We provide guidance on adding CI test cases for Huawei Ascend devices on verl.
...
For GPU test cases, the weights are stored in the `~/models/` directory. To adapt, you can use a symbolic link: ``ln -s /root/.cache/models ~/models``

The following is the original dataset. Process the data as needed. An example is shown below.
...
The content in the `${HOME}/.cache/` folder is not deleted when the container is destroyed after the CI run finishes. Avoid adding content to this folder.
...
(3) If the test path falls within the ``--ignore-glob`` exclusion scope, add a step in ``npu_unit_test.yml`` to explicitly run the test.
(4) If you add a batch of related test cases, it is recommended to create a dedicated workflow file to keep things clear.

4. Add the end-to-end test script
^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^

Steps:

(1) Create an end-to-end test script in the ``tests/special_npu/`` directory.
(2) In the ``.github/workflows/`` directory, find the workflow file that ends with ``_ascend.yml`` and is closest in functionality, and add a step to call the script in it.
...
* **Unit tests**: Cover core functions, classes, and methods to ensure logic correctness.
...
* **Resource management**: Multiple jobs in a workflow run in parallel. Set reasonable timeout values to avoid tasks hanging for a long time. Keep the running time of a single job within 40 minutes.
