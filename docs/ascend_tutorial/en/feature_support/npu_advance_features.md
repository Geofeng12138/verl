...
Ascend supports the vLLM inference backend through the **vllm-ascend plugin**. This plugin follows the [RFC](https://github.com/vllm-project/vllm/issues/11162) and provides pluggable interfaces to decouple Ascend NPUs from vLLM.
...
Megatron is a model-parallel training framework developed by NVIDIA. To run it on the NPU, you need to install **MindSpeed** for underlying support. MindSpeed uses **Monkey Patch** technology to seamlessly replace key Megatron components, enabling NPU adaptation.

#### MindSpeed Monkey Patch Framework Principles
...
└── Performs apply_features_pre_patches and apply_features_patches operations
```
...
|:---|:---|
| `Patch` class | Implements dynamic replacement of functions/classes and supports stacking multiple decorator layers |
| `parse_path()` | Performs dynamic module import and creation |
| `MindSpeedPatchesManager` | Manages all patch registrations as a global singleton |
| `MindSpeedFeature` | The Feature base class; each feature integrates the patch system through inheritance |
...
|:---|:---|
| `actor_rollout_ref.actor.megatron.override_transformer_config.deallocate_pipeline_outputs` | **Pipeline output deallocation** — Releases output data after tensors are sent to the next pipeline stage, reducing peak device memory usage. The default value is `False`. |
| `actor_rollout_ref.actor.megatron.override_transformer_config.recompute_granularity` | **Recomputation granularity control** — Supports `full`, `selective`, or `none`. `full` recomputes the entire Transformer layer, `selective` recomputes only the core attention parts, and the default value is `none`. |
| `actor_rollout_ref.actor.megatron.override_transformer_config.recompute_method` | **Recomputation method** — Requires `recompute_granularity=full`. Supports `uniform` or `block`, and the default value is `None`. |
| `actor_rollout_ref.actor.megatron.override_transformer_config.recompute_num_layers` | **Number of recomputation layers** — Requires `recompute_granularity=full`. A larger value reduces device memory usage but increases computational cost. The value must be divisible by the number of model layers in the current process. |
...
| `actor_rollout_ref.actor.megatron.override_transformer_config.use_fused_rotary_pos_emb` | **Fused Rotary Position Embedding** — Uses a fused operator to accelerate RoPE computation. The default value is `False`. |
| `actor_rollout_ref.actor.megatron.override_transformer_config.use_fused_swiglu` | **Fused SwiGLU** — Uses a fused operator to accelerate the SwiGLU activation function. The default value is `False`. |
...
| verl Parameter | Description |
|:---|:---|
| `actor_rollout_ref.actor.megatron.override_transformer_config.account_for_loss_in_pipeline_split` | **Loss layer pipeline partitioning** — Treats the loss layer as a standard Transformer layer in partitioning. The default is `False`. |
| `actor_rollout_ref.actor.megatron.override_transformer_config.account_for_embedding_in_pipeline_split` | **Embedding layer pipeline partitioning** — Treats the input embedding layer as a standard Transformer layer in partitioning. The default is `False`. |
...
| Memory saver mode (`enable_memory_saver`) | Inference (SGLang) | Supports dynamic release and restoration of device memory. The default value is `True` in verl. |
...
| Pipeline output release (`deallocate_pipeline_outputs`) | Training (Megatron) | Releases passed tensors in pipeline parallelism (PP) scenarios. |
...
| Feature | Inference/Training | Description |
|:---|:---|:---|
| Chunked prefill (`enable_chunked_prefill`) | Inference (vLLM) | Large prefill is chunked and processed together with the decode batch |
...
| Parallel Type | vLLM | SGLang | FSDP | Megatron | Description |
|:---|:---|:---|:---|:---|:---|
| Data Parallelism (DP) | ✅ | ✅ | ✅ | ✅ | Parallelism along the data dimension |
| Tensor Parallelism (TP) | ✅ | ✅ | — | ✅ | Tensor splitting within a layer |
| Pipeline Parallelism (PP) | — | — | — | ✅ | Pipeline splitting across layers |
| Expert Parallelism (EP) | ✅ | ✅ | — | ✅ | Parallelism along the MoE expert dimension |
...
- **Expert Parallelism (EP)** — Configured through the `ep_size` parameter, it assigns different experts to different NPU devices.
...
| verl parameter | Description |
|:---|:---|
| `actor_rollout_ref.actor.megatron.expert_model_parallel_size` | Expert parallelism (EP) size, default is `1` |
| `actor_rollout_ref.actor.megatron.expert_tensor_parallel_size` | TP-extended EP size, default is `null` |
| `actor_rollout_ref.actor.megatron.override_transformer_config.moe_grouped_gemm` | **Group GEMM** — uses Group GEMM to optimize expert computation in MoE scenarios, default is `False` |
| `actor_rollout_ref.actor.megatron.override_transformer_config.moe_router_dtype` | **Router data type** — data type for the weighted average of the router and expert outputs, optional `fp32`/`fp64`, default is `fp32`, improves stability in multi-expert scenarios |
...
3. **Recompute parameter dependencies**
   ...
   - When `recompute_method='uniform'`, `recompute_num_layers` specifies the number of Transformer layers in each recompute unit, and it must be divisible by the number of model layers in the current process.

4. **SGLang NPU-specific configuration**
   - `attention_backend` must be set to `ascend` to call the Ascend-optimized kernel
...
### Inference Backend Parameter Quick Reference
...
|:---|:---|:---|
| Parameter offloading | `fsdp_config.param_offload` | `megatron.param_offload` |
| Optimizer offloading | `fsdp_config.optimizer_offload` | `megatron.optimizer_offload` |
