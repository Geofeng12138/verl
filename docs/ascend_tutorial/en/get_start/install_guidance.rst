...
-  2025/12/11: The existing verl scenarios currently support automatic NPU device type detection. In principle, GPU scripts no longer need to explicitly set ``trainer.device=npu`` when running on Ascend; new features can still specify the device type preferentially by setting ``trainer.device``.

..

[Note] Automatic NPU device type detection requires that the environment where the program runs includes the ``torch_npu`` software package. If the environment does not include ``torch_npu``, you still need to explicitly specify ``trainer.device=npu``.
...
Currently, the following common training and inference backends are supported for deployment on the NPU. You can directly obtain the released images based on our `Ascend image description <dockerfile_build_guidance.rst>`__, or perform a custom installation as described below.
...
CANN          ``9.1.0``                               CANN software that helps developers build and run AI workloads on the Ascend software and hardware platform
...
transformers  ``5.10.4``                              Hugging Face large model library that provides model architectures and pretrained weights
...
MindSpeed     ``core_r0.16.0``                        Adaptation and optimization components for Megatron-LM on Ascend NPU
============= ======================================= ===================
...
CANN is the heterogeneous computing architecture on the NPU. The following are the installation commands for the A3 on the ARM platform. Download and install HDK and CANN by following the commands below, or download and install them from the `CANN community <https://www.hiascend.com/cann/download?versionId=723&ids=d803%2Ch0501%2Ch0601%2Ch0702>`_ based on your system hardware model.
...
Log filtering
^^^^^^^^^^^^^^^^^^^^^^^^
After upgrading the transformers version to 5.10.4, a large number of alias deprecation warnings may appear. You can add environment variables to filter redundant logs.
...
CANN           ``>=8.5.0``                            CANN software that helps developers build and run AI workloads on the Ascend software and hardware platform
...
transformers   ``5.3.0``                              Hugging Face large model library that provides model architectures and pretrained weights
Megatron-LM    ``core_r0.16.0``                       Large-scale distributed training framework
...
We provide a one-click deployment `installation script <../../../../scripts/install_sglang_mcore_npu.sh>`_ based on conda. The script installs the environment step by step. If an installation error occurs during the process, check the cause based on the error message of the current step, or leave us a message through an issue. We will resolve it as soon as possible.

.. code:: bash

# Note: When installing on an x86 platform, pip requires an additional source to be configured. Use the following command:
# pip config set global.extra-index-url "https://download.pytorch.org/whl/cpu/"
# Enable the CANN environment. If you have customized the CANN path, modify the following enable command based on your custom path.
source /usr/local/Ascend/ascend-toolkit/set_env.sh
source /usr/local/Ascend/nnal/atb/set_env.sh
...
# Support multiple processes on a single NPU
export HCCL_HOST_SOCKET_PORT_RANGE=60000-60050
export HCCL_NPU_SOCKET_PORT_RANGE=61000-61050

# Avoid Ray calling the device side, which cannot identify device availability based on the is_npu_available interface
...
The following Ascend ecosystem libraries are not yet supported in verl:
