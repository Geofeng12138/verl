...
Ascend hosts daily-built A2/A3 images in `quay.io/ascend/verl <https://quay.io/repository/ascend/verl?tab=tags&tag=latest>`_, which are built based on the `Dockerfile <../../../../docker/ascend>`_. For details, see :ref:`Dockerfile image build script list <ascend-dockerfile-list>`.
...
Dockerfile Image Build Script Checklist
---------------------------
...
* In the image with ``vLLM`` as the inference backend, vLLM, vLLM-ascend, MindSpeed, Megatron-LM, and verl are installed from source, and the source code is located in the ``/`` directory at the image root.
* In the image with ``SGLang`` as the inference backend, SGLang, MindSpeed, and verl are installed from source, and the source code is located in the ``/`` directory at the image root.
...
* For example, using the vLLM image, ``Dockerfile.ascend_8.5.0_a2`` is the Dockerfile file name. In ``verl-ascend:8.5.0-a2``, verl-ascend is the custom image name, and 8.5.0-a2 is the custom image tag.
...
* To mount other local paths to the container, add ``-v <host path>:<container path>`` yourself.
* We recommend that you replace ``{your_docker_name}`` with a meaningful container name.
...
Statement
--------------------
The Ascend-related Dockerfiles and images provided in verl are reference samples for trial purposes. If you plan to use them in a production environment, contact us through official channels. Thank you.
