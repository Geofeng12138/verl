...
Ascend hosts daily-built A2/A3 images in `quay.io/ascend/verl <https://quay.io/repository/ascend/verl?tab=tags&tag=latest>`_. These images are built using the `Dockerfile <../../../../docker/ascend>`_. For details, refer to :ref:`Dockerfile image build script list <ascend-dockerfile-list>`.
...
The verl release image name format: {verl release version number}-{CANN version}-{TorchNPU version}[-{applicable product information}-{operating system}]-{Python version}[-{inference backend}-{other fields}]



Image and Hardware Support
-----------------------------------
...
List of component version information in the latest image
----------------
...
* In images where the inference backend is ``vLLM``, vLLM, vLLM-ascend, MindSpeed, Megatron-LM, and verl are installed from source code located in the root directory ``/`` of the image.
* In images where the inference backend is ``SGLang``, SGLang, MindSpeed, and verl are installed from source code located in the root directory ``/`` of the image.


Image build command example
--------------------
...
* If you need to mount other local paths to the container, add ``-v <host machine path>:<path in the container>``
...
* Replace ``{image_name}:{tag}`` with the image name and tag used when building the container
...
Disclaimer
--------------------
The Ascend-related Dockerfiles and images provided in verl are reference samples for trial experience. If you use them in a production environment, communicate through official channels. Thank you.
