...
- 2026/06/30: Added coverage for four common training and inference backend combinations, making it easier for users to quickly select the appropriate startup script during the quickstart phase.
- 2026/05/13: Separated the quick start and install guidance.
- 2025/12/11: Existing verl scenarios now support automatic detection of NPU device types. GPU scripts running on Ascend no longer require explicitly setting the ``trainer.device=npu`` parameter in principle. The new feature can still be prioritized by setting ``trainer.device``, and the automatic detection capability is being gradually adapted.
...
This article provides a minimal GRPO training validation workflow based on GSM8K and Qwen3-0.6B for the Ascend NPU environment.

The documentation covers four commonly used training and inference backend combinations, making it easier for users to quickly select an appropriate startup script during the quickstart phase.

Before running the scripts in this document, ensure that you have completed the installation of the verl Ascend environment. For details on environment installation, see the `Ascend Installation Guide <./install_guidance.rst>`_.
...
The weights must be downloaded from Hugging Face by yourself.
...
The related scripts are all placed in the ``tests/special_npu/quick_start/`` directory.
...
For multi-node task startup, see the `Multi-Machine Task Startup Guide <https://github.com/verl-project/verl/blob/main/docs/ascend_tutorial/zh/model_support/examples/multi-machine_task_startup_practice.rst>`_.

SGLang Backend Enablement Guide
-------------------------------------------

Currently, verl has parsed common inference parameters. For details, see the ``ServerArgs`` initialization arguments in `async_sglang_server.py <../../../../verl/workers/rollout/sglang_rollout/async_sglang_server.py>`_.
