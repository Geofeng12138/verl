...
- 2026/06/30: Added coverage of four common training and inference backend combinations, making it easier for users to quickly select an appropriate startup script during the quickstart phase.
- 2026/05/13: Separated quick start and install guidance.
- 2025/12/11: Existing verl scenarios now support automatic detection of NPU device types. GPU scripts run on Ascend, and in principle, you no longer need to explicitly set the ``trainer.device=npu`` parameter. The new feature can still be prioritized by setting ``trainer.device``, with gradual adaptation to the automatic detection capability.
...
- `Qwen3-0.6B GSM8K GRPO Quick Start <#qwen3-06b-gsm8k-grpo-quick-start>`_
   ...
   - `Running Methods <#running-methods>`_
...
This article provides a minimal GRPO training verification workflow based on GSM8K and Qwen3-0.6B for the Ascend NPU environment.
...
Before running the scripts in this document, ensure that the verl Ascend environment installation is complete.
For environment installation details, refer to the `Ascend Installation Guide <./install_guidance.rst>`_.

A3 has 2 die per card, while A2 has 1 die per card. If you run the sample on an A3 machine, set ``n_gpus_per_node`` to 16.

All four scripts use the ``Qwen/Qwen3-0.6B`` model and the GSM8K dataset by default for basic pipeline validation.

This is mainly used to check:

- Whether the verl entry point is available;
- Whether the data can be read;
- Whether the actor, rollout, and reference workers can initialize;
...
Download the weights from Hugging Face yourself.

The default weight path read in the script is ``~/models/Qwen/Qwen3-0.6B``.
...
The relevant scripts are all placed under the ``tests/special_npu/quick_start/`` path.

First, enter the verl directory: ``cd /your/path/verl``

Enable the CANN environment: If you customized the CANN path, modify the following enabling command based on your custom path.
...
Quick Start currently provides four commonly used combinations of training and inference backends. You can select the corresponding script based on the training backend and the rollout backend.
...
For multi-node task startup, refer to the `Multi-Machine Task Startup Practice Guide <https://github.com/verl-project/verl/blob/main/docs/ascend_tutorial/zh/model_support/examples/multi-machine_task_startup_practice.rst>`_.

SGLang Backend Enablement Notes
-------------------------------------------

Currently, verl has parsed the common inference parameters. For details, see the ``ServerArgs`` initialization parameters in `async_sglang_server.py <../../../../verl/workers/rollout/sglang_rollout/async_sglang_server.py>`_.
